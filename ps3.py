...
class RectangularRoom(object):
    ...
    def __init__(self, width, height, dirt_amount):
        ...
        """
        assert width > 0 and height > 0 and dirt_amount >= 0, "RectangularRoom has wrong range of params"
        # assert all(isinstance(i, int) for i in [width, height, dirt_amount]), "RectangularRoom params aren't ints"

        if not all(isinstance(i, int) for i in [width, height, dirt_amount]):
    ...
    def clean_tile_at_position(self, pos, capacity):
        ...
        """
        intpos = (math.floor(pos.get_x()), math.floor(pos.get_y()))
        self.tiles[intpos] = max(0, self.tiles[intpos] - capacity)
...
class Robot(object):
    ...
    def __init__(self, room, speed, capacity):
        ...
        self.direction = random.uniform(0, 359.999)
    ...
    def set_robot_direction(self, direction):
        ...
        """
        assert 0 <= direction < 360, "new direction out of range"
...
class FurnishedRoom(RectangularRoom):
    ...
    def is_position_valid(self, pos):
        ...
        """
        if not self.is_position_furnished(pos) and RectangularRoom.is_position_in_room(self, pos):
            return True
        return False
    ...
    def get_random_position(self):
        ...
        while not self.is_position_valid(new_pos):
            new_pos = Position(random.uniform(0, self.width), random.uniform(0, self.height))
...
class StandardRobot(Robot):
    ...
    def update_position_and_clean(self):
        ...
        """
        # print("Original pos, dir, speed", self.position, "|", self.direction, self.speed)
        # new_pos = Position(self.position.get_x() + (self.speed*math.cos(math.degrees(90 - self.direction))),
        #                    self.position.get_y() + (self.speed*math.sin(math.degrees(90 - self.direction))))

        new_pos = self.position.get_new_position(self.direction, self.speed)
        # print("New pos, dir, speed", new_pos, "|", self.direction, self.speed)

        if self.room.is_position_valid(new_pos):
            self.position = new_pos
            self.room.clean_tile_at_position(self.position, self.capacity)
        else:
            self.set_robot_direction(random.uniform(0, 359.999))
...
class FaultyRobot(Robot):
    ...
    def update_position_and_clean(self):
        ...
        if self.gets_faulty():
            self.set_robot_direction(random.uniform(0, 359.999))
            # print("A", self.direction)
        elif self.room.is_position_valid(new_pos):
            self.position = new_pos
            self.room.clean_tile_at_position(self.position, self.capacity)
        else:
            self.set_robot_direction(random.uniform(0, 359.999))
...
def run_simulation(num_robots, speed, capacity, width, height, dirt_amount, min_coverage, num_trials,
    ...
    for trial in range(num_trials):
        ...
        # initializing each trial's robot and local shit
        local_trial_timesteps = 0
        trial_room = EmptyRoom(width, height, dirt_amount)
        robots = []
        for num_robot in range(num_robots):
            robots.append(robot_type(trial_room, speed, capacity))

        # anim = ps3_visualize.RobotVisualization(num_robots, width, height, isinstance(trial_room, FurnishedRoom), 0.05)

        while trial_room.get_num_cleaned_tiles() / trial_room.get_num_tiles() < min_coverage:
            for robot in robots:
                robot.update_position_and_clean()
            local_trial_timesteps += 1
            # anim.update(trial_room, robots)

        all_timesteps.append(local_trial_timesteps)
        # anim.done()

    return sum(all_timesteps)/len(all_timesteps)
...
# print("-----")
print ('avg time steps: ' + str(run_simulation(1, 1, 1, 20, 20, 5, 0.8, 50, StandardRobot)))
# print ('avg time steps: ' + str(run_simulation(1, 1, 1, 20, 20, 5, 0.8, 50, FaultyRobot)))
...
#       of a 20x20 room?
#
# FaultyRobot is a bit worse, but it gets less worse as more robots are added. This makes sense; as you add more robots
# the tons of robots cleaning leave less room for getting unlucky.
#
# 2) How does the performance of the two robot types compare when two of each
#       robot cleans 80% of rooms with dimensions 
#       10x30, 20x15, 25x12, and 50x6?
#
# As the aspect ratio goes up, the faulty robot does worse. Maybe because the faulty gets recursived on itself as it
# keeps hitting walls
